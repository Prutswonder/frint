...
var _ActionsObservable = __webpack_require__(5);
...
function Store() {
  ...
  if (this.options.epic) {
    this._input$ = new _rxjs.Subject();
    this._action$ = new _ActionsObservable2.default(this._input$);
    this._epic$ = new _rxjs.Subject();

    this._epicSubscription = this._epic$.map(function (epic) {
      return epic(_this._action$, _this, _this.options.thunkArgument);
...
var _combineEpics = __webpack_require__(8);
...
function combineEpics() {
  ...
  return function rootEpic() {
    ...
    return _rxjs.Observable.merge.apply(_rxjs.Observable, _toConsumableArray(toMerge));
  };
}
module.exports = exports['default'];
